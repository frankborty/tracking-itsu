...
namespace CAGPUUtils {
	...
	void Host::gpuMemcpyHostToDevice(void *dst, const void *src, int size)
	{
		cl_int iErr;
		//alloc memory object to device
		cl::Buffer buff=cl::Buffer(
				CAGPUContext::getInstance().getDeviceProperties().oclContext,
				CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, //device can read/write | device allocate memory and copy the content of src into it
				(size_t)size,
				(void*)src,
				&iErr);
		if(iErr!=CL_SUCCESS)
			throw std::runtime_error { CAGPUUtils::err_code(iErr) };
		dst=(void*)&buff;
	}
